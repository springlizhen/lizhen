...
public class DictionaryUtils {
     ...
     * @return
     */
    public static String setDicDataId(String key, HttpServletRequest request){
        List list = setDicDataListId(key, request);
        try {
            return JsonMapper.getInstance().writerWithView(DictData.SimpleView.class).writeValueAsString(list);
        } catch (JsonProcessingException e) {
            e.printStackTrace();
     ...
     * @return
     */
    public static List setDicDataListId(String key, HttpServletRequest request){
        Map<String, Object> mapParam = MapUtils.newHashMap();
        mapParam.put("dicTypeId", key);
        Result data = OALoginUtiles.authGet(request, mapParam,"/dicData/dicTypeId");
        List<Map> list = ListUtils.newArrayList();
        if(data.getData()!=null) {
            List<JSONObject> dictData = (List) JSON.parse(data.getData());
            for (JSONObject object : dictData) {
