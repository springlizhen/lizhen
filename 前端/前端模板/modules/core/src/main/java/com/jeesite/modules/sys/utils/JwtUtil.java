...
public class JwtUtil {
    ...
    private static long expires_in=3600000 ;//一个小时
    ...
    public static String createJWTPassword(String id, String subject, String password,String pm) {
        long nowMillis = System.currentTimeMillis();
        Date now = new Date(nowMillis);
        JwtBuilder builder = Jwts.builder().setId(id)
                .setSubject(subject)
                .setIssuedAt(now)
                .signWith(SignatureAlgorithm.HS256, secret).claim("password",password).claim("pm", pm);
        if (expires_in > 0) {
            builder.setExpiration( new Date( nowMillis + expires_in));
        }
        return builder.compact();
