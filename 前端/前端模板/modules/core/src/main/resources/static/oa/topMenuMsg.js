...
function unreadMsg(){
    $.get(ctx+"/msg/msgInner/listRecord?readStatus=2&__t="+new Date().getTime(),function(d){
        var b=$("#msgList").empty();
...
function pullPoolMsg(){
    ...
    $.get(ctx+"/msg/msgInner/listRecord?readStatus=2&__t="+new Date().getTime(),function(e){
        var b=$("#msgList").empty();
        for(i=0;i<e.length;i++){if(!(e.length>b)){
            ...
            var f={
                msgContentEntity:{
                    ...
                    content:js.text(a.attr("data-mergeMsgContent"),
                        e.length)
                },
                sendDate:"",
                sendUserName:"",
