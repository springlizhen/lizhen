...
;(function ($, window, document, undefined) {
  ...
  Calendar.prototype = {
    ...
    showCalendar: function (url) { // 输入数据并显示
      var self = this;
      ...
      this.$calendarDate_item.each(function (i) {
        ...
        if(dates==null){
          $.ajax({
            type: 'POST',
            url: url + "/sizeTFS?date="+allDay.toLocaleDateString(),
            dataType: 'json',
            async: false,
            error: function(data){
        ...
        $(this).attr('data', allDay_str);
        if($.inArray(allDay_str,dates)==-1)
          $(lit).html(allDay.getDate());
        else {
          $(lit).html(allDay.getDate() + '<div class="dian"></div>');
          $(lit).click(function () {
            js.addTabPage(null, '日程信息查询',
                url+"/listAll?time="+allDay.toLocaleDateString().replace(/\//g,"-"));
          });
        }
    ...
    renderDOM: function (url) { // 渲染DOM
      this.$calendar_title = $('<div class="calendar-title"></div>');
      this.$calendar_week = $('<ul class="calendar-week"></ul>');
      this.$calendar_date = $('<ul class="calendar-date" style="margin-left: 20px;"></ul>');
      this.$calendar_today = $('<div class="calendar-today"></div>');
      this.$calender_add = $('<div style="text-align: center;"></div>');


      var _titleStr = '<div>'+
						'<span class="arrow-prev left-bu"><</span>'+
					  '</div>' +
					  '<div style="text-align: center;"><a href="#" class="title blue"></a></div>'+
                      '<div class="arrow">'+
                        '<span class="arrow-next">></span>'+
                      '</div>';
      var _weekStr = '<li class="item blue">日</li>'+
                      '<li class="item blue">一</li>'+
                      '<li class="item blue">二</li>'+
                      '<li class="item blue">三</li>'+
                      '<li class="item blue">四</li>'+
                      '<li class="item blue">五</li>'+
                      '<li class="item blue">六</li>';
      var _dateStr = '';
      ...
      var _add = '<a data-href="'+url+'/form" href="javaScript:;" data-click-binded="true" class="btn btn-default btnTool addTabPage" style="background-color: rgb(2, 170, 245);color: white;width: 20%;" title="新增日程表" data-click-binded="true">新  增</a>';

      this.$calendar_title.html(_titleStr);
      this.$calendar_week.html(_weekStr);
      this.$calendar_date.html(_dateStr);
      this.$calendar_today.html(_dayStr);
      this.$calender_add.html(_add);

      this.$calendar.append(this.$calendar_title, this.$calendar_week, this.$calendar_date, this.$calendar_today,this.$calender_add);
      this.$calendar.show();
    },

    inital: function (url) { // 初始化
      var self = this;
  ...
  $.fn.calendar = function (options,url) {
    var calendar = new Calendar(this, options);

    return calendar.inital(url);
  };
