...
public class PdfExport extends PdfPageEventHelper {
     ...
     */
    public PdfExport(String yeMei, int presentFontSize, Rectangle pageSize) {
        this.header = yeMei;
    ...
    public void onEndPage(PdfWriter writer, Document document) {
        ...
        try {
            if (bf == null) {
                bf = BaseFont.createFont("C:\\font\\simfang.ttf", BaseFont.IDENTITY_H, BaseFont.NOT_EMBEDDED);
            }
            if (fontDetail == null) {
                fontDetail = new Font(bf, 14, Font.NORMAL);// 数据体字体
            }
        ...
        float len = bf.getWidthPoint(foot1, 14);
        ...
        if (pageS % 2 !=0) {
            ColumnText.showTextAligned(cb, Element.ALIGN_RIGHT, footer, (document.rightMargin() + document.right() + document.leftMargin() - document.left() - len) - 65F, document.bottom() - 50, 0);
        } else {
            ColumnText.showTextAligned(cb, Element.ALIGN_LEFT, footer, 95F, document.bottom() - 50, 0);
        }
        // 6.写入页脚2的模板（就是页脚的Y页这俩字）添加到文档中，计算模板的和Y轴,X=(右边界-左边界 - 前半部分的len值)/2.0F + len ， y 轴和之前的保持一致，底边界-20
        //cb.addTemplate(total, (document.rightMargin() + document.right() + document.leftMargin() - document.left()) / 2.0F + 10F, document.bottom()); // 调节模版显示的位置
